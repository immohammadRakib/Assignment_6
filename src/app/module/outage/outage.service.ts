...
const reportUnexpectedOutage = async (payload: any): Promise<any> => {
  ...
  if (!customer) {
    throw new Error(
      "Invalid Customer! No customer profile found with the provided ID.",
    );
  }
...
const resolveOutageJob = async (
  reportId: string,
  payload: { notes?: string },
): Promise<IOutageResolveResponse> => {
  return await prisma.$transaction(async (tx) => {
    ...
    const updatedReport = await tx.outageReport.update({
      ...
      data: {
        ...
        notes: payload.notes || "Resolved successfully by the technical team.",
      },
    });
...
const assignTechnicianManually = async (
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (report.status === OutageStatus.RESTORED) {
      throw new Error(
        "This outage ticket is already resolved and power is restored! Cannot assign a technician.",
      );
    }

    if (report.technicianId) {
      throw new Error(
        "A technician is already assigned to this outage ticket!",
      );
    }
...
const getAllTechniciansFromDB = async (
  filters: ITechnicianFilterableFields,
  options: any,
) => {
...
const getAllOutageReportsFromDB = async (
  filters: IOutageReportFilterableFields,
  options: any,
) => {
